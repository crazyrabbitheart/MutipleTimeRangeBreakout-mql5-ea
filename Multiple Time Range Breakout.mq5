//+------------------------------------------------------------------+
//|                                 Multiple Time Range Breakout.mq5 |
...
//|                                                             Loui |
//+------------------------------------------------------------------+
#property description "Loui - EA that helps to trade on news."
...
input group "Main"input group "Main"
//input datetime NewsDateTime = __DATE__; // NewsDateTime: Date and time of the news release.
input int MagicNumber = 1111; // Magic Number of this EA
input double TP = 150; // TP: Take-profit
input double SL = 50; // SL: Stop-loss
input double SDistance = 5; // SDistance: Fluff distance from recent SL in points
input double TDistance = 5; // TDistance: Fluff distance from recent TP in points
input double RR = 3.0; // RR: Risk-Reward
input int CTCBN = 10; // CTCBN: Number of candles to check before news for High & Low.
input int SecBPO = 300; // SecBPO: Seconds before news to place pending orders.
input int SecBMO = 150; // SecBMO: Seconds before news when to stop modifying orders.
...
input int BEPips = 5; // BEPips: Pips of profit when EA will move SL to breakeven + 1.
input int TrailingStop = 2; // Trailing Stop
...
input bool Fri = true;
input bool Sat = true;
input bool TradingSession1 = true;
input string str_session_001_start  = "13:30"; // Start time of trading session 1
input string str_session_001_end  = "14:00"; // End time of trading session 1
input bool TradingSession2 = false;
input string str_session_002_start  = "13:20"; // Start time of trading session 2
input string str_session_002_end  = "16:30"; // End time of trading session 2
input bool TradingSession3 = false;
input string str_session_003_start  = "15:20"; // Start time of trading session 3
input string str_session_003_end  = "21:30"; // End time of trading session 3

input group "Miscellaneous"
input int Slippage = 3;  // Slippage: Tolerated slippage in pips.
input string TradeLog = "Am_Log_"; // TradeLog: Log file prefix.
input string Commentary = "Loui"; // Commentary: trade description.
...
static datetime dt_tradingsession1_tstamp_start = 0;
static datetime dt_tradingsession1_tstamp_end = 0;
static datetime dt_tradingsession2_tstamp_start = 0;
static datetime dt_tradingsession2_tstamp_end = 0;
static datetime dt_tradingsession3_tstamp_start = 0;
static datetime dt_tradingsession3_tstamp_end = 0;
...
void OnInit()
{
    Magic = (int)MagicNumber; // Dynamically generated Magic number to allow multiple instances for different news announcements.
...
double LotsOptimized()
{
    ...
    if (MM)
    {
        ...
        lot = NormalizeDouble(MathFloor(AccountInfoDouble(ACCOUNT_MARGIN_FREE) * RiskPercent * 3 * 3.67 / (buy_stop_entry - buy_stop_loss) / 10000 / 100) / 100, LotStep_digits);
    }
...
void OpenBuyStop(string ctgr)
{
    for (int tries = 0; tries < 10; tries++)
    {
        Trade.OrderOpen(_Symbol, ORDER_TYPE_BUY_STOP, LotsOptimized(), 0, buy_stop_entry, buy_stop_loss, buy_take_profit, 0, 0, ctgr);
...
void OpenSellStop(string ctgr)
{
    for (int tries = 0; tries < 10; tries++)
    {
        Trade.OrderOpen(_Symbol, ORDER_TYPE_SELL_STOP, LotsOptimized(), 0, sell_stop_entry, sell_stop_loss, sell_take_profit, 0, 0, ctgr);
        ulong ticket = Trade.ResultOrder(); // Get ticket.
        if (ticket < 0)
        {
...
void DeleteBuyStop(string ctgr)
{
    for (int i = 0; i < OrdersTotal(); i++)
    {
        ...
        {
            if ((OrderGetString(ORDER_SYMBOL) == _Symbol) && (OrderGetInteger(ORDER_MAGIC) == Magic) && (OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_STOP) && (OrderGetString(ORDER_COMMENT) == ctgr))
            {
...
void DeleteSellStop(string ctgr)
{
    for (int i = 0; i < OrdersTotal(); i++)
    {
        ulong ticket = OrderGetTicket(i);
        if (OrderSelect(ticket))
        {
            if ((OrderGetString(ORDER_SYMBOL) == _Symbol) && (OrderGetInteger(ORDER_MAGIC) == Magic) && (OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_STOP) && (OrderGetString(ORDER_COMMENT) == ctgr))
            {
                if (!Trade.OrderDelete(ticket))
                {
...
void DoModify(string ctgr)
{
    for (int i = 0; i < OrdersTotal(); i++)
    {
        ...
        if ((OrderGetString(ORDER_SYMBOL) != Symbol()) || (OrderGetInteger(ORDER_MAGIC) != Magic) || OrderGetString(ORDER_COMMENT) != ctgr) continue;
...
void OnTick()
{
    ...
    datetime dt_current_tstamp = TimeCurrent();   
    
    TimeToStruct(dt_current_tstamp, current_time);
    if (current_time.day_of_week == 0 && Sun == false) return;
    if (current_time.day_of_week == 1 && Mon == false) return;
    if (current_time.day_of_week == 2 && Tue == false) return;
    if (current_time.day_of_week == 3 && Wed == false) return;
    if (current_time.day_of_week == 4 && Thu == false) return;
    if (current_time.day_of_week == 5 && Fri == false) return;
    if (current_time.day_of_week == 6 && Sat == false) return;    
    //printf(dt_current_tstamp);
    
    datetime StartTime;
    datetime EndTime;
    
    if (TradingSession1){
      StartTime = StringToTime(str_session_001_start);
      EndTime = StringToTime(str_session_001_end);
      doPerform(StartTime, EndTime, "01");
    }
    
    if (TradingSession2){
      StartTime = StringToTime(str_session_002_start);
      EndTime = StringToTime(str_session_002_end);       
      doPerform(StartTime, EndTime, "02");
    }
    
    if (TradingSession3){
      StartTime = StringToTime(str_session_003_start);
      EndTime = StringToTime(str_session_003_end);
      doPerform(StartTime, EndTime, "03");
    }      
}

void doPerform(datetime StartTime, datetime EndTime, string ctgr){
    ...
    buy_stop_entry = NormalizeDouble(recent_high + spread * Poin, _Digits);
    sell_stop_entry = NormalizeDouble(recent_low - spread  * Poin, _Digits);
    buy_stop_loss = NormalizeDouble(recent_low - SL * Poin, _Digits);
    sell_stop_loss = NormalizeDouble(recent_high + SL * Poin, _Digits);
    buy_take_profit = NormalizeDouble(buy_stop_entry + (RR * (buy_stop_entry - buy_stop_loss)) + TP * Poin, _Digits);
    sell_take_profit= NormalizeDouble(sell_stop_entry - (RR * (sell_stop_loss - sell_stop_entry)) - TP * Poin, _Digits);

    int sectonews = (int)(StartTime - TimeCurrent());
    Comment("\nLoui Expert Advisor",
            ...
            "\nCTCBN: ", CTCBN, " SecBPO: ", SecBPO, " SecBMO: ", SecBMO, " STWAN: ", STWAN, " OCO: ", OCO, " BEPips: ", BEPips, "CloseAfterSeconds: ", CloseAfterSeconds,
    ...
    if ((TimeCurrent() < StartTime) && (TimeCurrent() >= StartTime - SecBPO))
    {
    ...
    if ((TimeCurrent() < StartTime) && (TimeCurrent() >= StartTime - SecBPO) && (TimeCurrent() < StartTime - SecBMO))
    {
    ...
    if ((TimeCurrent() > StartTime) && (TimeCurrent() < StartTime + STWAN) && (OCO))
    {
